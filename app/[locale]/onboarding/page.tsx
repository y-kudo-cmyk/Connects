...
export default function OnboardingPage() {
  ...
  const handleVerifyCode = async () => {
    if (!code.trim()) { setCodeError(t('Onboarding.enterInviteCode')); return }
    setCodeLoading(true)
    setCodeError('')
    try {
      ...
      if (data.valid) {
        ...
        setCodeError(t('Onboarding.onboardingCodeInvalid'))
      }
    } catch {
      setCodeError(t('Onboarding.onboardingCodeFailed'))
    }
  ...
  return (
    ...
    >
      ...
      {step === 'lang' && (
        <>
          <div className="text-center mb-8">
            <p className="text-xl font-black mb-1" style={{ color: '#1C1C1E' }}>{t('Onboarding.onboardingLang')}</p>
            <p className="text-sm" style={{ color: '#8E8E93' }}>{t('Onboarding.onboardingLangSub')}</p>
          </div>
      ...
      {step === 'code' && (
        <>
          <div className="text-center mb-8">
            <p className="text-xl font-black mb-1" style={{ color: '#1C1C1E' }}>{t('Onboarding.onboardingCode')}</p>
            <p className="text-sm" style={{ color: '#8E8E93' }}>{t('Onboarding.onboardingCodeSub')}</p>
          </div>
          <div className="w-full max-w-sm flex flex-col gap-3">
            <input
              type="text"
              ...
              placeholder={t('Onboarding.onboardingCodePlaceholder')}
              maxLength={12}
              autoFocus
            ...
            >
              {codeLoading ? t('Onboarding.onboardingCodeConfirming') : t('Onboarding.onboardingCodeConfirm')}
            </button>
            <button
              ...
              style={{ color: '#8E8E93' }}
            >
              {t('Onboarding.onboardingBackToLang')}
            </button>
          </div>
        </>
      )}

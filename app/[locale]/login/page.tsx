...
export default function LoginPage() {
  ...
  const handleEmailLogin = async () => {
    ...
    try {
      ...
      setEmailError(e?.message || t('Auth.loginFailed'))
    }
    ...
    >
      ...
      <div className="w-full max-w-sm flex flex-col gap-3">
        <div className="text-center mb-2">
          <p className="text-sm" style={{ color: '#8E8E93' }}>{t('Auth.loginTitle')}</p>
        </div>
        ...
        <div className="relative flex items-center gap-3 my-1">
          ...
          <span className="text-xs" style={{ color: '#C7C7CC' }}>{t('Auth.loginEmailDivider')}</span>
          <div className="flex-1 h-px" style={{ background: '#E5E5EA' }} />
        </div>
        ...
        >
          {emailLoading ? t('Auth.loginLoading') : t('Auth.loginEmailButton')}
        </button>
      </div>

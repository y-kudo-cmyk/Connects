...
export async function GET(request: NextRequest) {
  ...
  const config = {
    ...
    pageFunction: [
      ...
      // リスナーは preNavigationHooks で登録済み (初期ロードを取りこぼさないため)。
      // ここでは userData から読み出すだけ。
      '  const ud = request.userData || {};',
      '  const cookieErrors = ud.cookieErrors || [];',
      '  const consoleLogs = ud.consoleLogs || [];',
      '  const allRequests = ud.allRequests || [];',
      '  const failedRequests = ud.failedRequests || [];',
    ...
    preNavigationHooks: [
      ...
      '  request.userData.consoleLogs = [];',
      '  request.userData.allRequests = [];',
      '  request.userData.failedRequests = [];',
      // ★ 重要: リスナーを navigation より先に登録 (pageFunction だと初期ロードを取りこぼす)
      '  page.on("console", (msg) => {',
      '    try {',
      '      const u = request.userData;',
      '      if (u.consoleLogs.length < 200) u.consoleLogs.push({ type: msg.type(), text: String(msg.text()).slice(0, 500) });',
      '    } catch(e) {}',
      '  });',
      '  page.on("pageerror", (err) => {',
      '    try {',
      '      const u = request.userData;',
      '      if (u.consoleLogs.length < 200) u.consoleLogs.push({ type: "pageerror", text: String(err && err.message || err).slice(0, 500) });',
      '    } catch(e) {}',
      '  });',
      '  page.on("request", (req) => {',
      '    try {',
      ...
      '      if (["xhr","fetch","script","document"].includes(rt) && u.allRequests.length < 300) {',
      '        u.allRequests.push({ url: req.url().slice(0, 300), method: req.method(), rt });',
      '      }',
      '    } catch(e) {}',
      '  });',

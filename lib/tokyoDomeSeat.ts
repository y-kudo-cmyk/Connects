...
import { TOKYO_DOME_BLOCKS, type DomeBlock } from './tokyoDomeData'
...
export function findBlock(blockId: string): DomeBlock | undefined {
 ...
 * row: 1 が内側 (中心寄り), 最大値が外側 (外周寄り)
 * seat: 1 が seatOrder='cw' のときブロック左端 (angleStart)
 */
 ...
 * A 系アーク: inner-start → outer-start → outer-end (arc) → inner-end → inner-start (arc)

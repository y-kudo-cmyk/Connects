...
function UploadModal({
  ...
  const distLabels = t.raw('Common.distanceLabels') as string[]
  const visLabels = t.raw('Common.visibilityLabels') as string[]
    ...
    <div className="fixed inset-0 z-[60] flex flex-col justify-end">
      ...
      <div className="relative flex flex-col rounded-t-2xl overflow-hidden" style={{ background: '#FFFFFF', maxHeight: '92vh' }}>
        ...
        <div className="flex items-center justify-between px-4 py-2" style={{ borderBottom: '1px solid #E5E5EA' }}>
          <p className="text-sm font-bold" style={{ color: '#1C1C1E' }}>{t('Seat.seatViewTitle')}</p>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
          {/* 写真 */}
          <div>
            <p className="text-xs font-bold mb-2" style={{ color: '#636366' }}>{t('Seat.seatViewDesc')}</p>
            {imageUrl ? (
              ...
              <button onClick={() => fileRef.current?.click()}
                ...
                <span className="text-sm">{t('Seat.seatViewSelect')}</span>
              </button>
            )}
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold" style={{ color: '#636366' }}>{t('Seat.seatViewBinoculars')}</p>
            <div className="flex gap-2">
              {[false, true].map((v) => (
                <button key={String(v)} onClick={() => setBinocularsNeeded(v)}
                  ...
                  {v ? t('Seat.seatViewNeeded') : t('Seat.seatViewNotNeeded')}
                </button>
              ))}
            </div>
          </div>
          ...
          {/* 評価 */}
          {[
            { label: t('Seat.seatViewDistance'), hints: DISTANCE_LABELS, val: distanceRating, setter: setDistanceRating, color: '#3B82F6' },
            { label: t('Seat.seatViewVisibility'), hints: VISIBILITY_LABELS, val: visibilityRating, setter: setVisibilityRating, color: '#F3B4E3' },
          ...
          <div>
            <p className="text-xs font-semibold mb-1.5" style={{ color: '#636366' }}>{t('Seat.seatViewMemoLabel')}</p>
            <textarea value={note} onChange={(e) => setNote(e.target.value)}
              placeholder={t('Seat.seatViewMemo')}
              rows={2}
        ...
        <div className="px-4 py-3" style={{ borderTop: '1px solid #E5E5EA', paddingBottom: 'calc(12px + env(safe-area-inset-bottom, 0px))' }}>
          <button onClick={handleSave} disabled={!imageUrl || saving}
            ...
            {saving ? t('Common.saving') : t('Seat.seatViewSubmit')}
          </button>
        </div>
      </div>
    </div>,
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <p className="text-xs font-bold flex items-center gap-1.5" style={{ color: '#636366' }}>
            ...
            {t('Seat.seatViewFromHere')}
            {matched.length > 0 && (
              <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full"
                style={{ background: 'rgba(59,130,246,0.12)', color: '#3B82F6' }}>
                {matched.length}{t('Map.countSuffix')}
              </span>
            )}
          </p>
        ...
        {matched.length === 0 ? (
          ...
          <button onClick={() => setShowUpload(true)}
            ...
            <p className="text-xs font-semibold" style={{ color: '#636366' }}>{t('My.noPostsYet')}</p>
            <p className="text-[11px]" style={{ color: '#8E8E93' }}>{t('My.noPostsHint')}</p>
          </button>
        ) : (
          <>
            {/* アリーナMAP */}
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: '#FFFFFF', border: '1px solid #E5E5EA' }}>
      ...
      <div className="relative" style={{ aspectRatio: '4/3' }}>
        ...
        <span className="absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded-full"
          ...
          {view.binocularsNeeded ? t('Seat.seatViewBinoYes') : t('Seat.seatViewBinoNo')}
        </span>
        {/* 距離バッジ */}
        {distLabel && (
      ...
      <div className="p-3 flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1">
            <span className="text-[11px]" style={{ color: '#8E8E93' }}>{t('Seat.seatViewDistance')}</span>
            <StarRating value={view.distanceRating} />
          </div>
          <div className="flex items-center gap-1">
            <span className="text-[11px]" style={{ color: '#8E8E93' }}>{t('Seat.seatViewVisibility')}</span>
        ...
        <p className="text-[10px]" style={{ color: '#C7C7CC' }}>
          {view.eventDate} · {view.contributor === '__self__' ? t('Map.arenaYou') : (view.contributor ?? t('Common.anonymous'))}
        </p>
      </div>
    </div>
  )

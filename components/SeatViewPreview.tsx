...
function UploadModal({
  ...
  const DISTANCE_LABELS = ['', '最前クラス', 'かなり近い', '普通', 'やや遠い', '遠い']
  const VISIBILITY_LABELS = ['', '見づらい', 'やや見づらい', '普通', '見やすい', '最高！']
    ...
    <div className="fixed inset-0 z-[60] flex flex-col justify-end">
      ...
      <div className="relative flex flex-col rounded-t-2xl overflow-hidden" style={{ background: '#FFFFFF', maxHeight: '92vh' }}>
        ...
        <div className="flex items-center justify-between px-4 py-2" style={{ borderBottom: '1px solid #E5E5EA' }}>
          <p className="text-sm font-bold" style={{ color: '#1C1C1E' }}>座席眺め写真を投稿</p>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
          {/* 写真 */}
          <div>
            <p className="text-xs font-bold mb-2" style={{ color: '#636366' }}>ステージが見える写真（着席後すぐ）</p>
            {imageUrl ? (
          ...
          <div>
            <p className="text-xs font-bold mb-2" style={{ color: '#636366' }}>
              アリーナ図で席の位置をタップ
              <span className="ml-1 font-normal" style={{ color: '#8E8E93' }}>（他のユーザーへの距離計算に使用）</span>
            </p>
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold" style={{ color: '#636366' }}>双眼鏡が必要でしたか？</p>
          ...
          {/* 評価 */}
          {[
            { label: '距離感', hints: DISTANCE_LABELS, val: distanceRating, setter: setDistanceRating, color: '#3B82F6' },
            { label: '見やすさ', hints: VISIBILITY_LABELS, val: visibilityRating, setter: setVisibilityRating, color: '#F3B4E3' },
          ...
          <div>
            <p className="text-xs font-semibold mb-1.5" style={{ color: '#636366' }}>一言メモ（任意）</p>
            <textarea value={note} onChange={(e) => setNote(e.target.value)}
              placeholder="柱が邪魔だった、花道が見えた、など..."
              rows={2}
      ...
      <div className="flex flex-col gap-3">
        ...
        {matched.length === 0 ? (
          ...
          <button onClick={() => setShowUpload(true)}
            ...
            <p className="text-xs font-semibold" style={{ color: '#636366' }}>まだ投稿がありません</p>
            <p className="text-[11px]" style={{ color: '#8E8E93' }}>参戦後に眺め写真を投稿してみんなに共有しよう！</p>
          </button>
        ) : (
          <>
            {/* アリーナMAP */}
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: '#FFFFFF', border: '1px solid #E5E5EA' }}>
      ...
      <div className="relative" style={{ aspectRatio: '4/3' }}>
        ...
        <span className="absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded-full"
          ...
          {view.binocularsNeeded ? '🔭 双眼鏡あると良い' : '✓ 双眼鏡不要'}
        </span>
        {/* 距離バッジ */}
        {distLabel && (
      ...
      <div className="p-3 flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1">
            <span className="text-[11px]" style={{ color: '#8E8E93' }}>距離感</span>
            <StarRating value={view.distanceRating} />
          </div>
          <div className="flex items-center gap-1">
            <span className="text-[11px]" style={{ color: '#8E8E93' }}>見やすさ</span>
        ...
        <p className="text-[10px]" style={{ color: '#C7C7CC' }}>
          {view.eventDate} · {view.contributor ?? '匿名'}
        </p>
      </div>
    </div>
  )
